...
## Как это должно работать сейчас и куда ведём

### Сейчас (текущий WebBridge MVP)

```text
Открываешь LinkedIn / X в браузере
  ↓
Расширение читает только видимые посты в активной вкладке
  ↓
В popup нажимаешь Scan Feed и Generate AI
  ↓
Мини-апп / очередь показывает варианты комментариев
  ↓
Ты подтверждаешь действие, расширение открывает пост и подставляет текст
  ↓
Финальную кнопку публикации/лайка/коннекта нажимаешь вручную
```

Важно: текущий MVP не является полноценным фоновым краулером. Он не обходит весь LinkedIn/X/Reddit сам по расписанию и не публикует действия без подтверждения пользователя. Расширение работает как безопасный браузерный мост: оно использует твою уже открытую веб-сессию, собирает видимые карточки и помогает подготовить действие.

### Целевое поведение

То, что ожидается как полноценный продукт:

- авторизоваться в веб-версиях LinkedIn, X и Reddit;
- установить расширение и связать его с Telegram Mini App;
- автоматически сканировать ленты/поиск/группы по расписанию, например каждые 5 минут;
- ранжировать посты по релевантности, популярности и свежести;
- показывать в Mini App карточку поста, формат контента, медиа, автора, метрики и варианты AI-комментариев;
- отправлять комментарии, лайки и LinkedIn connect invite только после ручного подтверждения.

Чтобы дойти до этого, нужны отдельные фоновые задания расширения, автопрокрутка/пагинация, общий нормализованный формат постов для LinkedIn/X/Reddit, дедупликация, скоринг и отдельная очередь подтверждений в Mini App.
