...
def verify_li_at(user_id: str, li_at: str) -> bool:
    try:
        client = Linkedin("", "", authenticate=False, cookies={"li_at": li_at}, proxies={"https": WEBSHARE_PROXY_URL} if WEBSHARE_PROXY_URL else {})
        ...
        logger.error("LinkedIn li_at verification failed user=%s err=%s", user_id, e)
    return False
